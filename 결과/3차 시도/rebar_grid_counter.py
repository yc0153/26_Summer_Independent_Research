...
# - target_image.jpg의 개구부 비율을 기준으로 후보를 고른다
# ------------------------------------------------------------
AUTO_GRABCUT_RECTS = [
    (0.05, 0.12, 0.90, 0.76),
    (0.05, 0.10, 0.90, 0.80),
...
# target_image.jpg에서 개구부가 보이는 위치를 비율로 적어 둔 기준값.
# 이미지 크기가 조금 달라져도 그대로 읽을 수 있게 픽셀이 아니라
# 가로/세로 비율로 저장해 두었다.
AUTO_OPENING_TARGET_FRACTIONS = np.array(
    [
        [0.1711, 0.3503],
        [0.6528, 0.2948],
        [0.9016, 0.6632],
...
# 2) 상/하/좌/우 후보를 따로 고른다
# 3) 네 직선의 교차점으로 사각형을 만든다
# 4) target_image.jpg의 개구부 비율과 가장 가까운 후보를 선택한다
# ------------------------------------------------------------
...
# 여기서 너무 많은 선을 남기면 조합 수가 급격히 늘어나므로
# 길이와 위치를 기준으로 먼저 걸러낸다.
def build_hough_pools(segments):
    pools = {
        "top": [
            s
            for s in segments
    ...
    for key in pools:
        if key == "top":
            pools[key] = sorted(
                pools[key],
                key=lambda s: (abs(s["my"] - 150), -s["length"]),
            )[:8]
        elif key == "bottom":
            pools[key] = sorted(
                pools[key],
                key=lambda s: (abs(s["my"] - 320), -s["length"]),
            )[:8]
        elif key == "left":
            pools[key] = sorted(
                pools[key],
                key=lambda s: (abs(s["mx"] - 120), -s["length"]),
            )[:8]
        else:
            pools[key] = sorted(
                pools[key],
                key=lambda s: (abs(s["mx"] - 430), -s["length"]),
            )[:8]
...
# target_image.jpg에 어울리는 개구부 모양과 가장 가까운 것을 고른다.
def detect_opening_quad_from_hough(image):
    height, width = image.shape[:2]
    target = AUTO_OPENING_TARGET_FRACTIONS * np.array([width, height], dtype=np.float32)
    edges, segments = detect_hough_segments(image)

    if not segments:
        raise RuntimeError("No Hough line segments found")

    pools = build_hough_pools(segments)
    ...
    for top, bottom, left, right in itertools.product(
        ...
        score = -float(np.sum(np.abs(ordered - target)))
