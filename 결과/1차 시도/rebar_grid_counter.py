...
def main():
    ...
    warped, perspective_matrix, ordered_points = warp_by_four_points(
        image, points, output_width=args.width
    )
    x_angle, y_angle, edges = estimate_grid_axes(warped)
    rectified, affine_matrix = affine_rectify_axes(warped, x_angle, y_angle)
    ...
    imwrite_unicode(save_dir / "02_perspective_warp.png", warped)
    imwrite_unicode(save_dir / "03_hough_edges.png", edges)
    imwrite_unicode(save_dir / "04_axis_rectified.png", rectified)
    imwrite_unicode(save_dir / "05_profiles.png", profile_plot)
    imwrite_unicode(save_dir / "06_count_overlay.png", overlay)
    ...
    with open(save_dir / "07_result.json", "w", encoding="utf-8") as file:
