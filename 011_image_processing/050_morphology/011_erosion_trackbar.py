...
kernel = cv2.getStructuringElement(shape=cv2.MORPH_ELLIPSE,
                                   ksize=(3, 3))
name_window = "erosion_iterations"
...
while True:
    ...
    result = cv2.erode(src=mask, kernel=kernel, iterations=iterations)

    preview = np.hstack(tup=[
        add_label(gray=mask, text="input"),
        add_label(gray=result, text=f"erosion x{iterations}")
    ])
    ...
    if key == ord("q"):
        ...
        path_save = os.path.join(dir_output, "07_erosion_trackbar.png")
