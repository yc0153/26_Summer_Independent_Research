...
kernel = cv2.getStructuringElement(shape=cv2.MORPH_ELLIPSE,
                                   ksize=(3, 3))
name_window = "opening_iterations"
...
while True:
    ...
    result = cv2.morphologyEx(src=mask, op=cv2.MORPH_OPEN,
                              kernel=kernel, iterations=iterations)

    preview = np.hstack(tup=[
        add_label(gray=mask, text="input"),
        add_label(gray=result, text=f"opening x{iterations}")
    ])
    ...
    if key == ord("q"):
        ...
        path_save = os.path.join(dir_output, "09_opening_trackbar.png")
