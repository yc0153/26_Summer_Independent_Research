...
kernel = cv2.getStructuringElement(shape=cv2.MORPH_ELLIPSE,
                                   ksize=(3, 3))
name_window = "closing_iterations"
...
while True:
    ...
    result = cv2.morphologyEx(src=mask, op=cv2.MORPH_CLOSE,
                              kernel=kernel, iterations=iterations)

    preview = np.hstack(tup=[
        add_label(gray=mask, text="input"),
        add_label(gray=result, text=f"closing x{iterations}")
    ])
    ...
    if key == ord("q"):
        ...
        path_save = os.path.join(dir_output, "10_closing_trackbar.png")
