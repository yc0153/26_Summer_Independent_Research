...
kernel = cv2.getStructuringElement(shape=cv2.MORPH_ELLIPSE,
                                   ksize=(3, 3))
name_window = "dilation_iterations"
...
while True:
    ...
    result = cv2.dilate(src=mask, kernel=kernel, iterations=iterations)

    preview = np.hstack(tup=[
        add_label(gray=mask, text="input"),
        add_label(gray=result, text=f"dilation x{iterations}")
    ])
    ...
    if key == ord("q"):
        ...
        path_save = os.path.join(dir_output, "08_dilation_trackbar.png")
