...
for contour in contours:
    ...
    kept += 1
    cv2.drawContours(image=result, contours=[contour], contourIdx=-1,
                     color=(0, 0, 255), thickness=2)
    x, y, _, _ = cv2.boundingRect(array=contour)
    cv2.putText(img=result, text=f"area={area:.0f}", org=(x, y - 8),
                fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.55,
                color=(0, 0, 255), thickness=2)

cv2.imwrite(filename=os.path.join(dir_output, "01_input_shapes.png"), img=img)
cv2.imwrite(filename=os.path.join(dir_output, "04_area_filter.png"),
            img=result)

print("Contours kept:", kept)
